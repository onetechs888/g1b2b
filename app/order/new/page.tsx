...
export default function OrderNewPage() {
  ...
  const totalTargetPrice = bomItems.reduce(
    (sum, item) => sum + item.targetPrice * item.qty,
    0,
  );
        ...
        <section className="px-8 py-7">
          <div className="mb-6 flex items-start justify-between">
            <div>
              <h1 className="text-2xl font-extrabold">
                발주 등록 <span className="text-lg">(타겟금액 기반)</span>
              </h1>
              <p className="mt-3 text-sm text-gray-600">
                고객이 목표금액과 BOM을 제시하고, 파트너사가 발주 참여를 요청하는 방식입니다.
              </p>
            </div>
          ...
          <section className="mb-4 rounded-2xl border border-gray-200 bg-white shadow-sm">
            <div className="border-b border-gray-200 px-6 py-4">
              <h2 className="text-lg font-extrabold">발주 프로젝트 정보</h2>
            </div>
            ...
            <div className="grid grid-cols-5 gap-5 p-6">
              ...
              <label className="space-y-2">
                <span className="text-xs font-bold text-gray-600">목표 총액</span>
              ...
              <label className="space-y-2">
                <span className="text-xs font-bold text-gray-600">발주 방식</span>
                <div className="flex h-11 items-center rounded-lg border border-gray-300 bg-[#fafafa] px-4 text-sm font-extrabold">
                  타겟금액 요청
                </div>
              </label>
              ...
              <label className="col-span-5 space-y-2">
                <span className="text-xs font-bold text-gray-600">프로젝트 설명</span>
                <input
          ...
          <section className="mb-4 rounded-2xl border border-gray-200 bg-white shadow-sm">
            <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4">
              <div>
                <h2 className="text-lg font-extrabold">BOM 품목 및 타겟금액</h2>
                <p className="mt-2 text-xs text-gray-500">
                  품목별 목표금액과 도면을 등록합니다. 파트너사는 해당 조건으로 참여 요청합니다.
                </p>
              </div>
              ...
              <table className="w-full min-w-[1360px] text-sm">
                <thead className="bg-[#fafafa] text-xs text-gray-500">
                  <tr>
                    ...
                    <th className="px-5 py-4 text-left">목표단가</th>
                    <th className="px-5 py-4 text-left">목표금액</th>
                    <th className="px-5 py-4 text-left">PDF</th>
                    <th className="px-5 py-4 text-left">DWG</th>
                      ...
                      <td className="px-5 py-4 font-bold">
                        {editingNo === item.no ? (
                          <input
                            ...
                            onChange={(e) => updateBomItem(item.no, "drawingNo", e.target.value)}
                            className="h-9 rounded border px-2"
                          />
                        ) : (
                          item.drawingNo
                      ...
                      <td className="px-5 py-4">
                        {editingNo === item.no ? (
                          <select
                            ...
                            onChange={(e) => updateBomItem(item.no, "postProcess", e.target.value)}
                            className="h-9 rounded border px-2"
                          >
                            <option>없음</option>
                            <option>아노다이징</option>
                      ...
                      <td className="px-5 py-4 font-bold">
                        {editingNo === item.no ? (
                          <input
                            ...
                            onChange={(e) =>
                              updateBomItem(item.no, "targetPrice", Number(e.target.value))
                            }
                            className="h-9 w-28 rounded border px-2 text-right"
                          />
                        ) : (
                          `${money(item.targetPrice)}원`
                        )}
                      </td>

                      <td className="px-5 py-4 font-extrabold">
                        {money(item.targetPrice * item.qty)}원
                      </td>

                      <td className="px-5 py-4">{item.pdf ? <FileIcon type="pdf" /> : "-"}</td>
                      <td className="px-5 py-4">{item.dwg ? <FileIcon type="dwg" /> : "-"}</td>
                      <td className="px-5 py-4">{item.step ? <FileIcon type="step" /> : "-"}</td>
          ...
          <section className="grid grid-cols-[1fr_420px] gap-4">
            <div className="rounded-2xl border border-gray-200 bg-white shadow-sm">
              <div className="border-b border-gray-200 px-6 py-4">
                <h2 className="text-lg font-extrabold">발주 조건 및 제조 요구사항</h2>
              </div>
              ...
              <div className="grid grid-cols-3 gap-5 p-6">
                ...
                <label className="col-span-3 space-y-2">
                  <span className="text-xs font-bold text-gray-600">발주 특이사항</span>
                  <textarea
                    value={orderMemo}
                    onChange={(e) => setOrderMemo(e.target.value.slice(0, 1000))}
                    className="h-24 w-full resize-none rounded-lg border border-gray-300 p-4 text-sm"
                    placeholder="목표금액 조건, 납기 우선순위, 부분 출하 조건 등을 입력하세요."
                  />
                  <p className="text-right text-xs text-gray-500">{orderMemo.length} / 1000</p>
                </label>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-gray-200 bg-white shadow-sm">
              <div className="border-b border-gray-200 px-6 py-4">
                <h2 className="text-lg font-extrabold">발주 설정</h2>
              </div>

              <div className="space-y-5 p-6">
                {[
                  ["allowPartialShipment", "부분 출하 허용"],
                  ["urgentOrder", "긴급 발주 요청"],
                  ["priorityInbound", "우선 입고 요청"],
                  ["g1Inspection", "G1 품질 검토 지원"],
                  ["g1Measure", "G1 3차원 측정 요청"],
                  ["ndaRequired", "NDA 필요"],
                  ["firstArticle", "초도품 검사 요청"],
                ].map(([key, label]) => (
                ...
                <label className="block space-y-2">
                  <span className="text-xs font-bold text-gray-600">파트너 요청 마감일</span>
                  <input
                    type="date"
                ...
                <label className="block space-y-2">
                  <span className="text-xs font-bold text-gray-600">파트너 전달사항</span>
                  <textarea
                    value={partnerMessage}
                    onChange={(e) => setPartnerMessage(e.target.value.slice(0, 500))}
                    className="h-20 w-full resize-none rounded-lg border border-gray-300 p-4 text-sm"
                    placeholder="참여 요청 파트너사에게 전달할 내용을 입력하세요."
                  />
                  <p className="text-right text-xs text-gray-500">{partnerMessage.length} / 500</p>
                </label>
              </div>
            </div>
          </section>
